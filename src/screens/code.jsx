...
function LoginCodeVerify() {
    ...
    const {id} = useParams(); // ✅ destructured email from URL
    ...
    // Redirect if no email param is present
    useEffect(() => {
        if (!id) {
    ...
    const submitHandler = async (e) => {
        ...
        const res = await dispatch(verifyLoginCode({email:id, code})); 

        setIsLoading(false);

        if (!res.bool) {
            setIsError(true);
            setIsErrorInfo("Invalid or expired code.");
            return;
        }
